...
export const getBookLimitItems = createAsyncThunk(
  "books/getBookLimitItems",
  async ({page, size}, thunkAPI) => {    const url = `${BASE_URL}/book/list?page=${page}&size=${size}`    
    try {
      const response = await axios.get(url);
      return response.data;
    } catch (error) {
...
export const getBooksByCategory = createAsyncThunk(
  ...
  async (categoryName, thunkAPI) => {
    const url =
      BASE_URL + "/book/searchBooks/category?categoryName=" + categoryName;
    try {
      const response = await axios.get(url);
      return response.data;
    } catch (error) {
...
export const getAllImagesByBookId = createAsyncThunk(
  "books/getAllImagesByBookId",
  async (id, thunkAPI) => {
...
const booksSlice = createSlice({
  ...
  initialState: {
    ...
    limit: 12, // Số sách trên mỗi trang
    images: [],
    message: "",
  },
  reducers: {
    resetImages(state) {
      state.images = []; // Xóa hình ảnh khi cuốn sách thay đổi
    },
    setPage(state, action) {
      state.currentPage = action.payload; // Thay đổi trang
    },
  },
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getBookLimitItems.rejected, (state, action) => {
        state.status = "failed";
        state.error = action.payload;
      });
  },
});
export const { resetImages, setPage} = booksSlice.actions;
